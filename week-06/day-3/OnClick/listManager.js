...
button1.onclick = () => { 
  ...
  for (let i = 0; i < elements.length; i++) {
    if (elements[i].getAttribute('style') === 'background-color : blue' &&  0 < parseInt(elements[i].getAttribute('id')) ) {
...
button3.onclick = () => {
  let elements = document.getElementsByTagName('li')
  for (let i = 0; i < elements.length; i++) {
    if (elements[i].getAttribute('style') === 'background-color : blue'){
      body.removeChild(elements[i])
      elements[0].setAttribute('style', 'background-color : blue')
    }
 }
}
